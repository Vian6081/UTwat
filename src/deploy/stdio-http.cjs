...
readline.createInterface({input:process.stdin}).on('line',line=>{
 ...
 const allowed=m.method==='GET' && ['/', '/health','/api/state','/api/runtime','/mock-instagram','/mock-instagram/','/mock-instagram/state','/mock-instagram/photo'].includes(m.path) || m.method==='POST' && ['/api/done','/mock-instagram/share'].includes(m.path);
