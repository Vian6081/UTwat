...
async function main(){
 ...
 process.env.AMMA_OFFLINE='true';process.env.AMMA_STATE_PATH=path.join(privateDir,'state.json');process.env.HOSTAGE_PHOTO=path.resolve('photos/aurafarmer.jpg');
 ...
 await new Promise(r=>setTimeout(r,2500));state.done=true;saveState(state);await tick();await page.waitForFunction(()=>document.getElementById('countdown')?.textContent==='Well done.');await new Promise(r=>setTimeout(r,2500));
 await context.close();await video.saveAs(path.join(out,'AMMA_offline_rehearsal.webm'));console.log('Recorded full escalation and release: artifacts/AMMA_offline_rehearsal.webm');
