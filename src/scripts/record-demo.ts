...
async function main(){
 ...
 await page.goto(`http://127.0.0.1:${(server.address() as any).port}/instagram`);await page.getByRole('button',{name:'Create',exact:true}).click();await page.getByRole('button',{name:'Share',exact:true}).waitFor();await page.screenshot({path:path.join(out,'mock-instagram.png'),fullPage:true});await new Promise(r=>setTimeout(r,3500));await page.goto(`http://127.0.0.1:${(server.address() as any).port}`);state.done=true;saveState(state);await tick();await page.waitForFunction(()=>document.getElementById('countdown')?.textContent==='Well done.');await new Promise(r=>setTimeout(r,2500));
